...
{
	//Usage information
	_tprintf( _T("ForceDelete 1.0 for WinNT/Win2k\n") );
	_tprintf( _T("\n") );
	_tprintf( _T("Deletes a given file even if it is locked by another process.\n") );
	_tprintf( _T("Written by Zoltan Csizmadia, zoltan_csizmadia@yahoo.com\n") );
	_tprintf( _T("\n") );
	_tprintf( _T("Usage: FORCEDEL.EXE [/S] filename\n") );
	_tprintf( _T("\n") );
	_tprintf( _T("          /S             Soft delete. Like the \"del\" command\n") );
	_tprintf( _T("          filename       File name you want to delete\n") );
	_tprintf( _T("\n") );
	_tprintf( _T("Examples:\n") );
	_tprintf( _T("\n") );
	_tprintf( _T("       FORCEDEL.EXE C:\\TEST.TXT\n") );
}
